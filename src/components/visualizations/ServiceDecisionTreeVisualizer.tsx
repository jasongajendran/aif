  ...
  const examScenarios = [
    {
      ...
      category: 'Document & Optical Character Recognition (OCR)',
      winningService: 'Amazon Textract',
      serviceIcon: FileText,
      serviceColor: 'text-amber-400 border-amber-500/40 bg-amber-950/20',
      reasoning: 'Amazon Textract goes beyond simple Optical Character Recognition (OCR) to extract tabular layouts, form relationships, and query values from scanned documents without manual Machine Learning (ML) training.',
      distractorTrap: 'Amazon Rekognition is for general images/faces/objects, NOT structured document form parsing. Amazon Comprehend extracts sentiment and Named Entity Recognition (NER) from raw text, not PDF layouts.',
      examTrigger: 'Key-value pairs, tables from PDFs, scanned forms ➔ Amazon Textract',
      relatedQuestions: [433, 441],
    },
    {
      id: 'pii-redaction',
      title: 'Redact social security numbers, medical records, and PII from customer chat logs',
      category: 'Natural Language Processing (NLP) & Named Entity Recognition (NER)',
      ...
      reasoning: 'Amazon Comprehend natively identifies and masks Personally Identifiable Information (PII) entities in unstructured text using Named Entity Recognition (NER). (If using Foundation Models [FMs], Amazon Bedrock Guardrails Layer 3 also masks PII in real time).',
      distractorTrap: 'Do not train a custom SageMaker Named Entity Recognition (NER) model when Amazon Comprehend provides pre-trained PII detection APIs out-of-the-box.',
      examTrigger: 'PII detection, sentiment analysis, Named Entity Recognition (NER) in raw text ➔ Amazon Comprehend',
      relatedQuestions: [425, 431],
    },
    {
      id: 'audio-transcribe',
      title: 'Convert recorded customer service phone calls into text transcripts',
      category: 'Speech & Audio (ASR)',
      winningService: 'Amazon Transcribe',
      serviceIcon: Mic,
      serviceColor: 'text-emerald-400 border-emerald-500/40 bg-emerald-950/20',
      reasoning: 'Amazon Transcribe uses Automated Speech Recognition (ASR) to convert speech-to-text with multi-channel audio support, Custom Vocabularies (teaching models domain jargon/acronyms), and Speaker Diarization (automated partitioning of audio to determine "who spoke when" e.g., Agent vs Customer).',
      distractorTrap: 'Amazon Polly is Text-to-Speech (TTS) [synthesizing voice], whereas Amazon Transcribe is Automated Speech Recognition (ASR) [speech-to-text].',
      examTrigger: 'Call center audio to text, speaker diarization ("who spoke when"), custom vocabulary ➔ Amazon Transcribe',
      relatedQuestions: [437],
    },
    {
      id: 'text-to-speech',
      title: 'Synthesize natural, human-like voice audio from written blog articles',
      category: 'Speech & Audio (TTS)',
      winningService: 'Amazon Polly',
      serviceIcon: Volume2,
      serviceColor: 'text-purple-400 border-purple-500/40 bg-purple-950/20',
      reasoning: 'Amazon Polly uses deep learning speech synthesis to convert text into lifelike spoken audio with Neural Text-to-Speech (NTTS), Speech Synthesis Markup Language (SSML tags for pauses, whispering, prosody, and phonemes), and custom Pronunciation Lexicons (PLS format for brand names).',
      distractorTrap: 'Amazon Transcribe converts audio to text; Amazon Polly converts text to audio.',
      examTrigger: 'Text to lifelike voice, neural speech synthesis, Speech Synthesis Markup Language (SSML tags such as <break>, <phoneme>, <whisper>) ➔ Amazon Polly',
      relatedQuestions: [437],
    },
    {
    ...
    },
    {
      ...
      category: 'Generative AI (GenAI) & Large Language Models (LLMs)',
      ...
      reasoning: 'Bedrock Knowledge Bases provides a fully managed Retrieval-Augmented Generation (RAG) pipeline that handles document parsing, semantic chunking, embedding, vector store synchronization, and model augmentation.',
      distractorTrap: 'Do not build a custom embedding cluster on Amazon EC2 or fine-tune model weights when Retrieval-Augmented Generation (RAG) satisfies the dynamic knowledge requirement.',
      examTrigger: 'Dynamic internal data, source citations, zero model weight modification ➔ Amazon Bedrock Knowledge Bases (RAG)',
    ...
    },
    {
      ...
      reasoning: 'Amazon Q Developer is the generative AI developer assistant integrated directly into VS Code, JetBrains Integrated Development Environments (IDEs), and AWS Console for code generation, vulnerability scanning, and code transformation.',
    ...
    },
    {
      ...
      serviceIcon: Shield,
      serviceColor: 'text-amber-400 border-amber-500/40 bg-amber-950/20',
      reasoning: 'SageMaker Model Cards are standardized digital fact sheets that centralize metadata, intended uses, risk ratings, and evaluation metrics for custom Machine Learning (ML) models throughout their lifecycle.',
      distractorTrap: 'AWS AI Service Cards document pre-trained AWS services (like Amazon Rekognition/Amazon Textract), whereas SageMaker Model Cards document CUSTOM models.',
    ...
    },
    {
      ...
      category: 'Governance & Human-in-the-Loop (HITL)',
      ...
      reasoning: 'Amazon Augmented AI (A2I) provides built-in Human-in-the-Loop (HITL) review workflows for machine learning models (Textract, Rekognition, or custom SageMaker models) whenever prediction confidence falls below a configured threshold.',
    ...
    },
    {
      ...
      category: 'Machine Learning Operations (MLOps) & Monitoring',
      ...
      distractorTrap: 'Amazon CloudWatch alarms monitor server CPU/latency, but SageMaker Model Monitor detects mathematical statistical data drift.',
      examTrigger: 'Detect data drift or concept drift on deployed endpoints ➔ SageMaker Model Monitor',
      relatedQuestions: [425, 440],
    },
  ];

  const categories = ['all', 'Document & Optical Character Recognition (OCR)', 'Natural Language Processing (NLP) & Named Entity Recognition (NER)', 'Speech & Audio (ASR)', 'Speech & Audio (TTS)', 'Computer Vision (CV)', 'Generative AI (GenAI) & Large Language Models (LLMs)', 'Developer Tools (IDE)', 'Governance & Auditing', 'Governance & Human-in-the-Loop (HITL)', 'Machine Learning Operations (MLOps) & Monitoring'];
